...
async function run() {
  try {
    ...
    app.post("/proposals", verifyToken, async (req, res) => {
      try {
        const { task_id, proposed_budget, estimated_days, cover_note, freelancer_email } =
          req.body;
    ...
    app.post("/payments", async (req, res) => {
      try {
        const { task_id, proposal_id, client_email, freelancer_email, amount } = req.body;
    ...
    app.patch("/users/:email", async (req, res) => {
      try {
        ...
        if (user.role !== "freelancer") {
          return res.status(403).json({ message: "Forbidden: Only freelancers can update profile details." });
        }
        const updateDoc = {
          $set: {
            name,
        ...
        if (result.matchedCount === 0) {
          return res.status(404).json({ message: "Failed to update database record." });
        }
    ...
    app.get("/proposals/client", async (req, res) => {
      try {
        ...
        const enriched = await Promise.all(
          proposals.map(async (p) => {
            const freelancer = await usersCollection.findOne({ email: p.freelancer_email });
            let taskTitle = "Unknown Task";
    ...
    app.get("/freelancer/stats", async (req, res) => {
      try {
        ...
        const earningsMetrics = await paymentsCollection
          ...
          .toArray();
        const stats = proposalMetrics[0] || { totalProposals: 0, pendingProposals: 0, acceptedProposals: 0 };
        res.json({
    ...
    app.get("/freelancer/active-projects", async (req, res) => {
      try {
        ...
        const projects = await tasksCollection
          .find({ freelancer_email: email, status: { $in: ["in_progress", "completed"] } })
          .sort({ createdAt: -1 })
          .toArray();
        res.json({ projects });
      } catch (error) {
    ...
    app.get("/admin/transactions", async (req, res) => {
      try {
        const transactions = await paymentsCollection.find({}).sort({ paid_at: -1 }).toArray();
